...
!      parameter        (ionmax=2)
      ...
      double precision pres, eint, cs, enthalpy, cp, cv, gammac, gammae
...
!      zbar   = abar * sum(xmass(1:ionmax) * zion(1:ionmax)/aion(1:ionmax))
  ...
  ! If no arguments, print usage and exit
      if (narg == 0) then
        print *, 'Usage: helm_program <dens> <temp> <abar> <zbar>'
        print *, 'Where:'
        print *, '  dens  = density rho (g/cm^3)'
  ...
  ! Read arguments and convert to double precision
      if (narg >= 4) then
...
! Invoke a simplified subroutine to call EOS
      call helm_wrapper (dens, temp, abar, zbar, pres, eint, cs, enthalpy, cp, cv, gammac, gammae)
...
!      print *, "eint + pres / dens = ",  eint + pres / dens
      ...
      print *, pres, " ", eint, " ", gammac, " ", gammae, " ", eint + pres / dens, " ", cs, " ", cp, " ", cv
